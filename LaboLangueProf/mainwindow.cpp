...
void MainWindow::loadImagesFromDB() {
    ...
    QSqlQuery query("SELECT Id_Eleve, Id_Activite FROM SessionEleve WHERE Id_Activite = 1");
    if (!query.exec()) {
    ...
    int maxPerRow = 6;    // Nombre max d'images par ligne
    ...
    while (query.next()) {
        ...
        CustomGraphicsItemGroup *group = new CustomGraphicsItemGroup(id, id_eleve);
